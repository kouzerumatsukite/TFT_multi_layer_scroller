...
// 32 pixels = 3 tiles
// 33 pixels = 4 tiles
static constexpr unsigned int TILES_SCREEN_WIDTH = TILES_ACROSS;
        ...
        for(unsigned int j = 0; j < 4; j++ ){
          ...
          for(unsigned int k = 0; k < 4; k++){
            ...
            if(debug_mode==2){
              for(int z = 0; z < 4; z++){
                bufferX[z]  |= 
                  0b0000000001000000 * (l == 1) | // Layer 1 no skip 4 pixels
                  0b0000000000000010 * (l == 2) | // Layer 2 no skip 4 pixels
                  0b0000100000000000 * (l == 3) ; // Layer 3 no skip 4 pixels
              }
            }
                ...
                for(unsigned o = 0; o < batchLen; o++){
                  switch(debug_mode){
                    ...
                    case 3: ///////////////////////////// VISUALIZE UNIQUE VS CLONE
                      ...
                      break;
                    case 4: { /////////////////////////// VISUALIZE FAST VS SLOW DRAW
                      bool z = directWrite | o         ;
                      bufferX[tileBatches[o]] |= z  ?  // -------------------------
                      0b1000001000010000 * (l == 0) |  // Layer 0 fast tile color
                      0b0000000001001000 * (l == 1) |  // Layer 1 fast tile color
                      0b0110000000000010 * (l == 2) |  // Layer 2 fast tile color
                      0b0000100100000000 * (l == 3) :  // Layer 3 fast tile color
                      0b0000010000100001 * (l == 0) |  // Layer 0 slow tile color
                      0b0000000010000000 * (l == 1) |  // Layer 1 slow tile color
                      0b0000000000000100 * (l == 2) |  // Layer 2 slow tile color
                      0b0001000000000000 * (l == 3) ;  // Layer 3 slow tile color
                      break; }
                    case 5: ///////////////////////////// VISUALIZE DEPTH
                      if(!bufferX[tileBatches[o]]){
                      bufferX[tileBatches[o]]    = 0  |  // ---------------
  ...
  if (Serial.available() > 0) {
    ...
    switch(command){
      case 0: // debug mode selector
        if (payload > 0){
          debug_enable = true;
          const char *debug_menus[7] = {
            "visualize normal",
            "visualize overdraw",
            "visualize skipping pixels",
            "visualize unique vs clone tile render",
            "visualize fastpath",
            "visualize depthmap",
            "gray scale mode",
          };
          ...
          const uint32_t debugMenuCount = sizeof(debug_menus) / sizeof(debug_menus[0]);

          if (payload >= 1 && payload <= debugMenuCount) {
            debug_enable = true;
            debug_mode = payload - 1;
            Serial.printf("debug mode is adjusted to: %s\n", debug_menus[debug_mode]);
          } else if (payload != 0) Serial.println("invalid debug mode");
        }else{
          debug_enable = false;
      ...
      case 1: // layer options
        ...
        if(command<3){
          if (payload < 4) {
            ...
            Serial.println("invalid layer index");
          }
        }else
        {
          if(payload < 4){
      ...
      case 2: { // tile engine rendering optimizations
        ...
        if(command<3){
          if( payload < 3 ){
            switch(command){
              case 0 : bool_options[payload] = !bool_options[payload]; break; //toggle
              case 1 : bool_options[payload] = false;  break; // disable option
              case 2 : bool_options[payload] = true;  break; // enable option
            }
            ...
            Serial.println("invalid tile optimization option");
          }
        }else
        {
          if(payload < 3){
      ...
      case 3: { // screen rendering options
        ...
        const char*name_options[3] = { "interlace", "double_buffer" };
        ...
        if( command < 3 ){
          if( payload < 2 ){
            ...
            interlace = bool_options[0];
            double_buffer = bool_options[1];
            Serial.printf("%s is %s\n", name_options[payload], bool_options[payload] ? "enabled":"disabled");
          }else{
            Serial.println("invalid screen rendering option");
          }
        }else
        {
          if(payload < 2){
